...
* 选择 `dist/pose_capture-1.3.2-legacy.zip`
* 在列表里勾选启用 **Pose Capture (DWPose to Rigify)**

### 方式 B：4.2+ 扩展机制（推荐，实测通过）

把 `dist/pose_capture-1.3.2-extension.zip` 解压到（目录名必须是 `pose_capture`）：
...
8. **躯干刚性偏移**：肩、胯、颈、头基座相对躯干的静止偏移会随重建出的躯干方向一起旋转（否则锁骨、脖子方向会明显偏）
9. **朝向归一**：按“人物朝向”把整套坐标旋到骨架静止坐标系（角色面朝 -Y），于是左/右关键点天然对应 `.L` / `.R`
10. **重定向**：
...
* **脸部**：只用眼睛、眉毛、下巴估计头部基准与滚转，不做表情/面骨
