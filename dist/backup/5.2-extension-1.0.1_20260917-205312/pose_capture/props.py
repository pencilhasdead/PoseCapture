...
class PC_Settings(PropertyGroup):
    ...
    arm_flip_L: BoolProperty(
        name="左臂前后翻转", default=False,
        description="单张图看不出肘部在身体前面还是后面；只把左臂改成“手肘朝前、手腕朝后”")
    arm_flip_R: BoolProperty(
        name="右臂前后翻转", default=False,
        description="单张图看不出肘部在身体前面还是后面；只把右臂改成“手肘朝前、手腕朝后”")
...
def options_from_settings(settings: PC_Settings) -> dict:
    return {
        ...
        "arm_flip_L": settings.arm_flip_L,
        "arm_flip_R": settings.arm_flip_R,
