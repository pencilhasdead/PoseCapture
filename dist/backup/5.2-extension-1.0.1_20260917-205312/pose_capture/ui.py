...
class PC_PT_main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.prop(settings, "use_depth")
        if settings.use_depth:
            row = col.row(align=True)
            row.prop(settings, "knee_front")
            row.prop(settings, "elbow_back")
            row = col.row(align=True)
            row.prop(settings, "arm_flip_L")
            row.prop(settings, "arm_flip_R")
            col.prop(settings, "spine_lean_front")
        row = col.row(align=True)
