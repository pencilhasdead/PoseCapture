...
def _normalize_gray(values, percentiles: bool = True) -> np.ndarray:
    """深度 -> 灰度图。默认按 2%~98% 百分位拉伸，避免个别异常值把整张压黑 / 压白。"""
    ...
    norm = np.clip((clean - low) / span, 0.0, 1.0)
...
def build_depth_view(settings, out_dir: str, keypoints=None, scores=None) -> dict:
    ...
    near_is_large = bool(depth_info.get("near_is_large", True))
    thr = float(settings.score_thr)

    photo = rgb
    if keypoints is not None and scores is not None:
        photo = imops.draw_pose_preview(rgb, keypoints, scores, thr)
    gray = _normalize_gray(depth_map, True)
    ...
    items = (
        ("photo", "输入照片（含检测骨架）", photo, photo,
         "先确认检测到的人是对的"),
        ("depth_gray", "深度灰度（2%~98% 拉伸）", gray_rgb, depth_map,
         "越白 = 越%s" % ("近" if near_is_large else "远")),
        ("depth_color", "深度伪彩（近=红 远=蓝）", color, depth_map,
         "和照片对照：脸/手这些离镜头近的地方应该是红的"),
    )
    panels = []
