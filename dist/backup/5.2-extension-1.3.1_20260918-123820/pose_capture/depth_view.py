...
以及**深度方向核对**（物理判据“人比背景近” vs 重建拟合出来的方向，不一致就说明前后反了）。

配色约定（整个插件统一）：灰度**越白 = 越远**，伪彩**近 = 蓝、远 = 红**。
...
def format_stats(manifest: dict) -> str:
    ...
    if direction.get("text"):
        lines.append("方向核对：%s" % direction["text"])
        if direction.get("fit") is None:
            lines.append("    （还没点“应用到骨架”，暂时只有物理判据可看）")
        elif not direction.get("agree"):
            lines.append("    ✗ 拟合出来的方向和“人比背景近”相反：整套姿态的前后可能反了，"
                         "勾上“反转方向”再算一次")
        lines.append("")
