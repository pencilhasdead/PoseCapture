...
背景（Blender 5.2 + 内置 rigify 实测）：
  ...
  * Rigify 自带的 “IK->FK (hand.L)” 按钮本质是
    ``pose.rigify_limb_ik2fk_<rig_id>``（``RigifyLimbIk2FkBase.apply_frame_state``）：
    读 FK 链的姿态矩阵，算出 IK 控制器的变换。
    实测把 FK 姿态搬过去、再把 IK_FK 切回 0 之后，所有 DEF 骨的位置误差 0.000000 m，
    手指 / 脚趾也全部跟随。

于是插件写姿态的顺序是：

* **IK 模式（默认）**：直接按上面那套算法把姿态写进 IK 控制器——把 FK 链每根骨骼的**世界增量**
  （位置 + 旋转）整体套到对应控制器上（``hand_ik`` / ``foot_ik`` 本身就是 IK 链末端，Rigify 用一根
  带 ``COPY_LOCATION`` 的 MCH 骨把它接到链上），链根 ``upper_arm_ik`` / ``thigh_ik`` 再按 Rigify 的
  ``correct_rotation`` 搜一遍绕自身轴的扭转（IK 链的弯曲平面）；开了极向目标（``pole_vector``）
  就用 :func:`place_pole` 把 ``*_ik_target`` 摆进 FK 链的平面；FK 链也顺手写一份（IK 模式下它是
  隐藏的，用户拖滑块回 FK 时姿态不丢，直写也要拿它当增量来源）。写完用 ``DEF-*`` 变形骨核对
  （关节位置 2 cm / 方向 10°；不比尾巴，IK 带出来的 DEF 骨会被 stretch 拉长），对不上的肢体自动
...
def ik_write_plan(spec: dict) -> dict | None:
    ...
    "tip_role", "tip_fk"}``：
      ...
      * ``controls`` 放“能对上 FK 角色、本身又是控制器”的 IK 骨——链根 ``upper_arm_ik.L`` /
        ``thigh_ik.L``、末端 ``hand_ik.L`` / ``foot_ik.L``（**它就是 IK 链末端**，位置 + 旋转都要写）
        以及尾控制器 ``toe_ik.L``；中间段由 IK 解算决定，不写；
      * ``target`` 是 ``*_ik_target`` —— 它其实是**极向目标**（决定 IK 链的弯曲平面），只有开了
        ``pole_vector`` 才按 Rigify 的 ``match_pole_target`` 摆它（见 :func:`place_pole`）；
      * ``tip_role`` / ``tip_fk``：这条肢体“终点”的角色与 FK 骨（``forearm_fk`` 的尾巴是
        手腕、``shin_fk`` 的尾巴是脚踝）；直写不需要它（写的是控制器自己的增量），留着给自检看。
    """
...
def _update_view() -> None:
    """让依赖图重算一遍（IK 解算 / 约束结果都靠它生效）。"""
    view_layer = bpy.context.view_layer
    if view_layer is not None:
        view_layer.update()


def pole_enabled(arm_obj, spec: dict) -> bool:
    """这条肢体现在用不用“极向目标”（Rigify 面板上的 ``pole_vector`` 开关）。"""
    prop = spec.get("pole_prop") or "pole_vector"
    pb = arm_obj.pose.bones.get(spec.get("prop_bone") or "")
    try:
        return bool(pb is not None and prop in pb.keys() and pb[prop])
    except (TypeError, ValueError):  # noqa: BLE001
        return False


def _perpendicular(vec):
    """Rigify ``perpendicular_vector``：随便挑一个跟 ``vec`` 垂直的方向（不保证归一）。"""
    other = Vector((1.0, 0.0, 0.0)) if abs(vec[0]) < abs(vec[1]) else Vector((0.0, 1.0, 0.0))
    return vec.cross(other)


def _rotation_difference(mat1, mat2) -> float:
    """Rigify ``rotation_difference``：两个旋转最短路径的**有符号**夹角（弧度）。"""
    q1, q2 = mat1.to_quaternion(), mat2.to_quaternion()
    angle = math.acos(min(1.0, max(-1.0, q1.dot(q2)))) * 2.0
    if angle > math.pi:
        angle = -angle + 2.0 * math.pi
    return angle


def place_pole(arm_obj, spec: dict, match_matrix) -> bool:
    """把极向目标（``*_ik_target``）摆进 FK 链所在的平面。

    IK 链的弯曲平面是极向目标定的：不摆它（留上一次的位置）新姿态里膝盖 / 手肘会朝错方向
    —— 实测右腿差 0.21 m / 26°，摆对之后 1.4° 以内。算法从 Rigify 的 ``match_pole_target``
    抄的：先拿“链根 -> 链末”的连线，在它中间往外挑一条垂线放杆；再看链根朝向跟 FK 差多少，
    沿连线正反各转一次，取更接近 FK 的那边。

    返回是否真的摆了。``match_matrix`` 是 FK 链首骨的姿态矩阵。
    """
    ik_names = spec.get("ik_bones") or []
    ctrl_names = spec.get("ctrl_bones") or []
    if len(ik_names) < 2 or len(ctrl_names) < 2:
        return False
    ik_first = arm_obj.pose.bones.get(ik_names[0])
    ik_last = arm_obj.pose.bones.get(ik_names[1])
    pole = arm_obj.pose.bones.get(ctrl_names[1])
    if ik_first is None or ik_last is None or pole is None:
        return False

    a = ik_first.matrix.to_translation()
    b = ik_last.tail.copy()          # == Rigify 的 ik_last.matrix.translation + ik_last.vector
    ikv = b - a
    if ikv.length < 1e-6:
        return False
    offset = _perpendicular(ikv).normalized() * (ik_first.bone.length + ik_last.bone.length)

    def set_pole(vector) -> None:
        matrix = pole.matrix.copy()
        matrix.translation = a + (ikv / 2.0) + vector
        pole.matrix = matrix
        _update_view()

    set_pole(offset)
    angle = _rotation_difference(ik_first.matrix, match_matrix)
    plus = Matrix.Rotation(angle, 4, ikv) @ offset
    set_pole(plus)
    angle_plus = _rotation_difference(ik_first.matrix, match_matrix)
    minus = Matrix.Rotation(-angle, 4, ikv) @ offset
    set_pole(minus)
    angle_minus = _rotation_difference(ik_first.matrix, match_matrix)
    if angle_plus < angle_minus:
        set_pole(plus)
    return True
