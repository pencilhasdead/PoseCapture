...
def _write_limb_ik_controls(arm_obj, plan: dict, targets: dict, refs: dict, metrics: dict,
                            to_arm, obj_mat_inv, use_twist: bool, twist_blend: float) -> None:
    """IK 控制器：链根 / 末端写旋转，IK 目标写位置。"""
    rest_refs = metrics.get("ref") or {}

    # 1) 控制器写旋转：链根决定整条 IK 链的姿态与弯曲平面，末端决定手 / 脚的朝向。
    #    静止矩阵取“FK 链对应骨骼”的 —— Rigify 的 IK 链与 FK 链静止方向一致，这样写出来的
    #    世界朝向就是 FK 模式下的世界朝向（Rigify 的 “IK->FK” 快照做的也是这件事）。
    for role, control in plan["controls"].items():
        bone = arm_obj.data.bones.get(plan["fk_of_role"].get(role) or "")
        ...
        if mat3 is None:
            continue
        # 只写旋转：控制器自己的位置照旧（IK 目标的位置另外单独写）
        pbone.matrix = Matrix.Translation(pbone.head.copy()) @ mat3.to_4x4()
        _update_view()

    # 2) IK 目标写位置：把手腕 / 脚踝搬到“刚写好的 FK 链末端”那里。用相对位移而不是绝对坐标，
    #    Rigify 内部 MCH 目标骨相对控制器的偏移就自动带过去了（不用管它怎么搭的）；
    #    而且根骨位移 / 父级姿态也已经体现在 FK 链末端上（根运动模式下同样对）。
    tip_target = targets.get(plan["tip_role"])
    tip_bone = arm_obj.data.bones.get(plan["tip_fk"])
    tip_pbone = arm_obj.pose.bones.get(plan["tip_fk"])
    target_bone = arm_obj.data.bones.get(plan["target"])
    pbone = arm_obj.pose.bones.get(plan["target"])
    if tip_bone is not None and target_bone is not None and pbone is not None:
        if tip_pbone is not None:
            base = tip_pbone.tail.copy()
        elif tip_target is not None:
            # 没写成 FK 链（例如那条链缺骨）时退回关键点，至少有个合理位置
            base = obj_mat_inv @ Vector(tip_target[1])
        else:
            base = None
        if base is not None:
            delta = base - Vector(tip_bone.tail_local)
            matrix = pbone.matrix.copy()
            matrix.translation = Vector(target_bone.head_local) + delta
            pbone.matrix = matrix
            _update_view()
...
def _ik_direct_check(arm_obj, spec: dict, roles) -> tuple | None:
    ...
    （调用方按“没通过”处理，退回实测误差 0.000000 m 的老路径）。
    """
    ...
    for fk_name in spec.get("fk_bones") or []:
        ...
        if fk_pb is None or def_pb is None:
            continue
        fk_head, fk_tail = matrix @ fk_pb.head, matrix @ fk_pb.tail
        df_head, df_tail = matrix @ def_pb.head, matrix @ def_pb.tail
        worst_pos = max(worst_pos, (fk_head - df_head).length, (fk_tail - df_tail).length)
        fk_dir, df_dir = fk_tail - fk_head, df_tail - df_head
...
def apply_ik_direct(arm_obj, plans: dict, targets: dict, refs: dict, metrics: dict,
    ...
    obj_mat_inv = arm_obj.matrix_world.inverted()
    ...
    for key in ready:
        ...
        try:
            _write_limb_ik_controls(arm_obj, plan, targets, refs, metrics, to_arm,
                                    obj_mat_inv, use_twist, twist_blend)
            _update_view()
