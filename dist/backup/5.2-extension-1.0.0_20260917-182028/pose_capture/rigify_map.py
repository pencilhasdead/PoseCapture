...
def auto_detect(arm_obj, include_fingers: bool = True) -> BoneMap:
    """在骨架上自动匹配角色骨名。"""
    ...
    for role, _label, cands in ROLE_DEFS:
        if "{s}" in role:
            ...
            for side in ("L", "R"):
                ...
                resolved = _pick(name_set, [cand.format(s=side) for cand in cands])
                if resolved:
...
def _align_sign(vec: Vector, reference: Vector) -> Vector:
    if reference.length < 1e-9:
        return vec
    return -vec if vec.dot(reference) < 0.0 else vec
...
def read_metrics(arm_obj, bmap: BoneMap) -> dict:
    ...
    for side in ("L", "R"):
        ...
        if upper and fore:
            ...
            if axis.length > 1e-6:
                metrics["ref"]["armbend.%s" % side] = _align_sign(
                    axis.normalized(), metrics["shoulder_line"])
        if fore and hand:
            axis = (Vector(fore.tail_local) - Vector(fore.head_local)).cross(
                Vector(hand.tail_local) - Vector(hand.head_local))
            if axis.length > 1e-6:
                metrics["ref"]["wristbend.%s" % side] = _align_sign(
                    axis.normalized(), metrics["shoulder_line"])
        if thigh and shin:
            axis = (Vector(thigh.tail_local) - Vector(thigh.head_local)).cross(
                Vector(shin.tail_local) - Vector(shin.head_local))
            if axis.length > 1e-6:
                metrics["ref"]["kneebend.%s" % side] = _align_sign(
                    axis.normalized(), metrics["hip_line"])
