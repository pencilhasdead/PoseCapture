    ...
    for side, per_hand in recon.get("fingers", {}).items():
        for finger, chain in per_hand.items():
            ...
            for index, base in enumerate(names):
                targets["%s.%s" % (base, side)] = (chain[index], chain[index + 1], None)
    return targets
...
def solve_orientation(rest_mat3: Matrix, target_dir: Vector, ref: Vector = None,
    ...
    # 找到与 rest_ref 最贴合的静止轴（自动适配不同骨架的 roll 约定）
    rest_ref_n = rest_ref.normalized()
    best_axis, best_dot = 0, 0.0
    for index in range(3):
        dot = rest_mat3.col[index].normalized().dot(rest_ref_n)
        if abs(dot) > abs(best_dot):
            best_axis, best_dot = index, dot
    if abs(best_dot) < 0.2:
        return matrix

    axis = target_dir.normalized()
    v1 = (matrix.col[best_axis] * (1.0 if best_dot > 0.0 else -1.0)).normalized()
    projected = ref - axis * ref.dot(axis)
    if projected.length < 1e-6:
        return matrix
    v2 = projected.normalized()
    if v1.dot(v2) < 0.0:
        v2 = -v2

    angle = math.atan2(v1.cross(v2).dot(axis), v1.dot(v2))
...
def apply_pose(arm_obj, bmap, metrics, recon, options=None) -> dict:
    ...
    for role, (p_from, p_to, ref_name) in targets.items():
        ...
        if ref_name:
            ...
            if ref_vec is not None and float(getattr(ref_vec, "length", 0.0)) > 1e-9:
