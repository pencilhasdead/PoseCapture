...
def _align_sign(vec: np.ndarray, reference: np.ndarray) -> np.ndarray:
    if float(np.linalg.norm(reference)) < 1e-9:
        return vec
    return -vec if float(np.dot(vec, reference)) < 0.0 else vec
...
def reconstruct(keypoints, scores, metrics, image_size, options=None) -> dict:
    ...
    for side in ("L", "R"):
        ...
        wrist = solve(elbow, pts["wr_%s" % side], metrics["forearm_len"][side], elbow_back)
    ...
    for side in ("L", "R"):
        ...
        hand = joints["hand_tip.%s" % side] - joints["wrist.%s" % side]
        thigh = joints["knee.%s" % side] - joints["hip.%s" % side]
        shin = joints["ankle.%s" % side] - joints["knee.%s" % side]
        axis = _safe_cross(upper_arm, forearm)
        if axis is not None:
            refs["armbend.%s" % side] = _align_sign(axis, refs["shoulder_line"])
        axis = _safe_cross(forearm, hand)
        if axis is not None:
            refs["wristbend.%s" % side] = _align_sign(axis, refs["shoulder_line"])
        axis = _safe_cross(thigh, shin)
        if axis is not None:
            refs["kneebend.%s" % side] = _align_sign(axis, refs["hip_line"])
